        ...
        if key=='stream:stream':
            if not sessions(self.sessionL, client).get('auth'):
                if not sessions(self.sessionL, client).get('tls'):
                    ...
                    self.send(client, '<stream:features><compression xmlns="http://jabber.org/features/compress"><method>zlib</method></compression></stream:features>', False)
                    sessions(self.sessionL, client).put('tls', True)
                else:
                    self.send(client, f'''<?xml version='1.0' encoding='UTF-8'?><stream:stream xmlns:stream="http://etherx.jabber.org/streams" xmlns="jabber:client" from="127.0.0.1" id="{sessions(self.sessionL, client).get("id")}" xml:lang="und" version="1.0"><stream:features><compression xmlns="http://jabber.org/features/compress"><method>zlib</method></compression><ver xmlns="urn:xmpp:features:rosterver"/></stream:features>''')
            else:
            ...
            self.send(client, '<proceed xmlns="urn:ietf:params:xml:ns:xmpp-tls"/>xml:ns:xmpp-tls"><compression xmlns="http://jabber.org/features/compress"><method>zlib</method></compression><ver xmlns="urn:xmpp:features:rosterver"/></stream:features>')
            sessions(self.sessionL, client).put('tls', True)
            ...
            if not sessions(self.sessionL, client).get('auth'):
                ...
                self.send(client, f'''<?xml version='1.0' encoding='UTF-8'?><stream:stream xmlns:stream="http://etherx.jabber.org/streams" xmlns="jabber:client" from="127.0.0.1" id="{sessions(self.sessionL, client).get("id")}" xml:lang="und" version="1.0"><stream:features><compression xmlns="http://jabber.org/features/compress"><method>zlib</method></compression><ver xmlns="urn:xmpp:features:rosterver"/></stream:features>''')
                self.send(client, f'''<success xmlns="urn:ietf:params:xml:ns:xmpp-sasl"/><stream:stream xmlns:stream="http://etherx.jabber.org/streams" xmlns="jabber:client" from="127.0.0.1" id="{sessions(self.sessionL, client).get("id")}" xml:lang="und" version="1.0"><stream:features><compression xmlns="http://jabber.org/features/compress"><method>zlib</method></compression><ver xmlns="urn:xmpp:features:rosterver"/></stream:features>''')
            ...
            if status['Properties']==[]:
                ...
                self.send(client, f'''<iq type="result" id="8" from="{sessions(self.sessionL, client).get("username")}@oldmp.software" to="{sessions(self.sessionL, client).get("jid")}"/>{sessions(self.sessionL, client).get("resource")}</jid></bind></iq><stream:features><compression xmlns="http://jabber.org/features/compress"><method>zlib</method></compression><ver xmlns="urn:xmpp:features:rosterver"/><bind xmlns="urn:ietf:params:xml:ns:xmpp-bind"/><session xmlns="urn:ietf:params:xml:ns:xmpp-session"><optional/></session></stream:features>''')
                self.send(client, f'''<presence corr-id="{corrid}" from="{sessions(self.sessionL, client).get("username")}@oldmp.software" to="{sessions(self.sessionL, client).get("jid")}"><status>{status}</status><priority>0</priority><delay xmlns="urn:xmpp:delay" stamp="{datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")}"></delay></presence><optional/></session></stream:features>''')
