...
function getItemShop() {
    const catalog = JSON.parse(JSON.stringify(require("./../connect/catalog.json")));
...
function getTheater(req) {
    ...
    var theater = JSON.stringify(require("./../connect/worldstw.json"));
    ...
    try {
        ...
        theater = theater.replace(/2022-12-14T23:59:59.999Z/ig, date);
    } catch (err) {}
...
function getContentPages(req) {
    ...
    const contentpages = JSON.parse(JSON.stringify(require("./../connect/contentpages.json")));
    ...
    const modes = ["battleRoyale", "saveTheWorld"];
    const news = ["battleroyalenews"]
    const motdnews = ["battleroyalenews"]
    ...
    try {
        if (memory.build < 5.30) { 
            news.forEach(mode => {
                contentpages[mode].news.messages[0].image = "https://cdn.discordapp.com/attachments/735984479906037845/1020084668512620544/unknown_69_1.png";
                contentpages[mode].news.messages[1].image = "https://cdn.discordapp.com/attachments/735984479906037845/1020085432660271166/channels4_profile_1_1.jpg";
            })
        }
    } catch (err) {}
